...
# Allow user to set max pages, try to prevent Rapid requests can trigger anti-bot protections. Change 100 to max, can be any number like 1000 or 10000000
try:
...
try:
    while urls:
        ...
        for anchor in soup.find_all("a", href=True):
            ...
            # Only follow links within the same domain and avoid duplicates
            if urlparse(link).netloc == target_domain and link not in urls and link not in scraped_urls:
                urls.append(link)
